...
search.addEventListener('keydown', (e) => {
    if (!(e.code === 'ArrowDown' || e.code === 'ArrowUp' || (id > 0 && e.code === 'Enter')))
        return false;
    if (e.code === 'ArrowDown')
        id = id < searchList.length ? id + 1 : 1;
    if (e.code === 'ArrowUp')
        id = id > 1 ? id - 1 : searchList.length;
    let domElements = document.querySelectorAll('.stock-box');
...
function getSpecialExp(word) {
    ...
    if (searchIdx === -1)
        ...
        return word.replace(spacialExp, '\\$&');
}
...
es.addEventListener('message', e => {
    ...
    let middle = StockList.map(m => m.price).sort((a, b) => a - b)[Math.ceil(prices.length / 2)];
